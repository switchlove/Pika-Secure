...
function computeRiskScore(member, guildConfig, burstCount, avatarReuseCount = 0) {
  const reasons = [];
  let score = 0;
  ...
  if (SUSPICIOUS_USERNAME_PATTERN.test(member.user.username)) {
    ...
    reasons.push(`Username matches a common bot-generated pattern — +${SUSPICIOUS_USERNAME_POINTS}`);
  }
  ...
  if (trustedBadges.length > 0) {
    ...
    reasons.push(`Account has trusted public badge(s) (${trustedBadges.join(', ')}) — -${TRUSTED_BADGE_DISCOUNT}`);
  }
